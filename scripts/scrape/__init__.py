...
if __name__=="__main__":
    ...
    parser.add_argument('-n','--num',default=20,required=False)
    parser.add_argument('-u','--num_per', default=1,required=False) # remove duplicates, so actual count is lower
    ...
    show_ids=set((x.__id__ for x in shows))
    ...
    all_ratings=[list(filter(lambda x:x[1] in show_ids,user_ratings(user.__username__,user.__username__))) for user in users]
